...
from monitoring.json_cache import JsonDiskCache, cache_dir, cache_key

json_converter = make_converter()

_cache = JsonDiskCache(cache_path=f"{cache_dir}/run_code_cache.json")
...
class CompiledProgram:
    ...
    async def run_input(
        ...
        key = cache_key(
            json.dumps(dict(code_hash=self.code_hash, inp=inp, timeout_seconds=timeout_seconds, memory_limit_mb=memory_limit_mb), sort_keys=True),
            "code_exec_no_c",
        )
        ...
        if not override_cache:
            ...
            if cache_result is not None:
                return json_converter.structure(cache_result, CommandOut)

        out = await simple_command_run(
            self.binary_path, timeout_seconds=timeout_seconds, memory_limit_mb=memory_limit_mb, inp=inp
        )
        _cache.set(key, json_converter.unstructure(out, unstructure_as=CommandOut))
        return out
    ...
    async def run_inputs(
        ...
        key = cache_key(
            json.dumps(dict(code_hash=self.code_hash, inps=inps, timeout_seconds=timeout_seconds, memory_limit_mb=memory_limit_mb), sort_keys=True),
            "code_exec_many",
        )
        ...
        if not override_cache:
            cache_result = _cache.get(key)
            if cache_result is not None:
                return json_converter.structure(cache_result, list[CommandOut])
        ...
        _cache.set(key, json_converter.unstructure(out, unstructure_as=list[CommandOut]))
        return out
